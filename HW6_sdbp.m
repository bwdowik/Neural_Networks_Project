...
batch_size=5;
alpha = 0.1*batch_size; %scale alpha by batch size
k=1; %iteration counter
target = double(G(p_vals)); %determine target values

%random initial weights, will change each time, do not use for testing
% W1(:,k) = [rand(1)-0.5; rand(1)-0.5];
% b1(:,k) = [rand(1)-0.5; rand(1)-0.5];
% W2(k,:) = [rand(1)-0.5 rand(1)-0.5];
% b2(k) = [rand(1)-0.5];

% might be better to use these for testing, for consistency
W1(:,k) = [-0.27; -0.41];
b1(:,k) = [-0.48; -0.13];
W2(k,:) = [0.09 -0.17];
b2(k) = [0.48];
...
%         end
    ...
    %weight updates
    W2(k+1,:) = W2(k,:)-alpha*W2sum/batch_size;
    b2(k+1) = b2(k)-alpha*b2sum/batch_size;
    W1(:,k+1) = W1(:,k)-alpha*W1sum/batch_size;
    b1(:,k+1)=b1(:,k)-alpha*b1sum/batch_size;
